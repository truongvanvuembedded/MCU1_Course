...
void UART_Init(UART_Handle_t *pUARTHandle)
{
	...
	pUARTHandle->pUARTx->BRR = u4_UART_BaudRate_Cal(pUARTHandle->UART_Config.BaudRate, u4_UART_SystemClk(pUARTHandle)); // Set baud rate
...
static uint32_t u4_UART_BaudRate_Cal(uint32_t BaudRate, uint32_t UARTClk)
{
    ...
    // Calculate USARTDIV
    f4_UsartDiv = (float)UARTClk / (BaudRate * 16);
    
    // Extract Mantissa and Fraction
    u4_UsartDivMantissa = (uint32_t)f4_UsartDiv;
    u4_UsartDivFraction = (uint32_t)((f4_UsartDiv - u4_UsartDivMantissa) * 16);
